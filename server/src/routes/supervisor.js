...
import { getAssignedServices } from "../db.js"; // alias válido a listServicesByUser

const router = Router();

/**
 * GET /supervisor/services
 * Devuelve los servicios asignados al usuario autenticado (supervisor).
 */
router.get("/services", [requireAuth, requireRole("supervisor")], (req, res) => {
  try {
    const userId =
      req.user?.id ?? req.auth?.id ?? req.userId ?? null;
