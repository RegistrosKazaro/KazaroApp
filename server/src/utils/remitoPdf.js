...
const fmtDateTime = (s) => {
  ...
  try {
    const d =
      typeof s === "string" ? new Date(s.replace(" ", "T")) : new Date(s);
...
function drawHeader(doc, nro, fecha, total) {
  ...
  doc
    ...
    .fontSize(22)
    .fill("#000")
    .text(money(total), xR + 100, M + 60 + 6, {
      width: RIGHT_W - 110,
      align: "right",
      lineBreak: false,
    });
...
function drawItemsTable(doc, y, rows, total) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (i % 2 === 1)
      doc.save().rect(M, y - 1, contentW, rowH).fill("#FBFBFC").restore();
    const it = data[i];
    let xx = M;
...
function drawFooter(doc) {
  ...
  doc
    ...
    .fontSize(9)
    .fill("#777")
    .text(`Generado por Kazaro ${nowLocal()}`, M, y, {
      width: doc.page.width - M * 2,
      lineBreak: false,
    });
}
